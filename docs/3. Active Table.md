...
### Extended Fields with Joins

Often a joined table will have duplicated fields with the base table. By default, the duplicated fields are made unique with the joined table name and an underscore prepended to the field name.

For example, if you join the **InventoryTransaction** and **Product** classes, you will produce a *product_product_id* field. To suppress this, set \PHPFUI\ORM::$extendedFields to false, or use \PHPFUI\ORM\Table::setExtendedFields() to disable it for a specific table.
