...
**SQLite** does not support the PDO::rowCount() method for counting the number of rows returned for reads. This means obtaining Array, DataObject and Record cursors should be done via the Table object. This will correctly return the correct number of rows for the query. Use of the static cursor functions (listed below) will not return the correct row count, but will otherwise interate through all the rows correctly.
