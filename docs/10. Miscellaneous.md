...
## Porting from older versions of this library
- V1 -> V2
...
- V3 -> V4
	- Added support for driver specific versions of PDO (Pdo\*) to avoid PHP 8.5 depreciations.
	- **\PHPFUI\ORM\PDOInstance** will continue to use the generic PDO driver.
  - Use **\PHPFUI\ORM\PDO\Factory**::get() to create a **PDOInstance** inherited from a specific Pdo\* driver.
  - Added **\PHPFUI\ORM\Interface\PDOInstance** for iusing the above Factory. Replace any **\PHPFUI\ORM\PDOInstance** with **\PHPFUI\ORM\Interface\PDOInstance**.
  - **PDOInstance->postGre** and **PDOInstance->sqlite** public properties have been replaced by getPostGre() and getSqlite() methods.
...
**SQLite** does not support the PDO::rowCount() method for counting the number of rows returned for reads. This means obtaining Array, DataObject and Record cursors should be done via the Table object. This will return the correct number of rows for the query. Use of the static cursor functions (listed below) will not return the correct row count, but will otherwise interate through all the rows correctly.
...
Returns an ArrayCursor from any SQL statement. **NOTE:** count() is not supported for this construct in SQLite

### getDataObjectCursor(string $sql = 'select 0 limit 0', array $input = []) : \PHPFUI\ORM\DataObjectCursor
Returns an DataObjectCursor from any SQL statement. **NOTE:** count() is not supported for this construct in SQLite

### getRecordCursor(\PHPFUI\ORM\Record $crud, string $sql = 'select 0 limit 0', array $input = []) : \PHPFUI\ORM\RecordCursor
Returns an RecordCursor from any SQL statement. **NOTE:** count() is not supported for this construct in SQLite
...
## Support Namespaces
The following namespaces are deployed in production code but only contain support classes or examples.
